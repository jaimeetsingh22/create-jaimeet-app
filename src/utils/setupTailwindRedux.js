...
export async function setupTailwindRedux(projectPath, isTS) {
  const spinner = makeSpinner(
    "🎨 Installing Tailwind CSS and Redux..."
  ).start();
  ...
  try {
    ...
    if (isTS) {
      fs.writeFileSync(
        mainTsxPath,
        `${uiData.redux.main.ts}\n`
      );   
    } else {
      fs.writeFileSync(
        mainJsxPath,
        `${uiData.redux.main.js}\n`
      );
    }
  ...
  const appPath = path.join(projectPath, "src", isTS ? "App.tsx" : "App.jsx");
    fs.writeFileSync(
        appPath,
        `${isTS ? uiData.combinations.tailwind_redux.App.ts : uiData.combinations.tailwind_redux.App.js}\n`
      );
